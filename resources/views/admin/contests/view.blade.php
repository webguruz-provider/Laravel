...
<form method="POST" action="/partners/contests/edit/{{$editad->id}}">
      ...
      <div class="ranged">
        <div class="field field__left">
          <div class="ad-img"><img src="{{ asset('img-dist/contests/'.$editad->image_url) }}" /></div>          
        </div>
      ...
      <span>Author: {{ Auth::user()->name }}</span> <span>Published: {{ \Carbon\Carbon::parse($editad->publish_at)->format('M d, Y')}} </span><span>
        <form method="POST" action="/partners/contests/deactivate/{{$editad->id}}">
                        <input type="hidden" name="ad_id" value="{{$editad->id}}">
